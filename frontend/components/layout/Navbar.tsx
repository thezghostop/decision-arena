    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-[#1e1e2e]">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-2">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          ...
          <span className="font-bold text-white hidden sm:block">Decision Arena</span>
        </Link>
        ...
        <div className="flex items-center gap-1 min-w-0">
          {links.map(({ href, label, icon: Icon }) => (
            <Link
              ...
              title={label}
              className={cn(
                "flex items-center gap-1.5 px-2 sm:px-4 py-2 rounded-lg text-sm transition-colors",
                pathname === href
          ...
          {/* Language switcher */}
          <div className="flex items-center gap-0.5 ml-1 bg-[#111118] border border-[#1e1e2e] rounded-lg p-0.5">
            {LOCALES.map((l) => (
              <button
                key={l.code}
                ...
                title={l.label}
                className={cn(
                  "px-1.5 sm:px-2 py-1 rounded-md text-xs font-medium transition-colors",
                  locale === l.code
          ...
          <button
            ...
            className={cn(
              "ml-1 flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-medium border transition-all",
